...
import {getRootIndexKey, rootIndexBucket, storeRootIndex} from "./stateArchiveIndex.js";
...
export class StateArchiveRepository extends Repository<Slot, BeaconStateAllForks> {
  ...
  private async getSlotByRoot(root: Root): Promise<Slot | null> {
    const value = await this.db.get(getRootIndexKey(root), {bucketId: rootIndexBucket});
